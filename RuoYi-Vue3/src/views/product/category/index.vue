<template>
  <div class="app-container">
    ...
    <el-table v-if="refreshTable" v-loading="loading" :data="categoryList" row-key="id" :default-expand-all="isExpandAll"
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button size="small" link :icon="Plus" @click="handleAdd(scope.row)"
            v-hasPermi="['product:category:add']">新增子类</el-button>
    ...
    <!-- 添加或修改商品分类对话框 -->
    <el-dialog align-center :title="title" v-model="open" width="680px" append-to-body>
...
export default {
  name: "Category",
  setup() {
    return { Search, Refresh, Plus, Delete, Download, Sort, Edit };
  },
  data() {
    return {
      // 遮罩层
  ...
  watch: {
    'form.parentId': 'handleParentIdChanged'
  },
  methods: {
    /** 查询商品分类列表 */
    getList() {
    ...
    /** 查询新增商品分类的默认排序 */
    getNextSortNum(id) {
      var categoryId = id == null ? 0 : id;
      getNewCategorySort(categoryId).then((response) => {
        this.form.sort = response.data || 0;
      });
    },
    // 取消按钮
    cancel() {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除商品分类名为【' + categoryName + '】的数据项及其子项？')
        .then(function () {
    ...
    /** 处理上级商品分类改变操作 */
    handleParentIdChanged(val) {
      if (val == null) {
        return;
      }
      if (this.form.id == null) {
        this.getNextSortNum(val)
      }
    },
  },
};
