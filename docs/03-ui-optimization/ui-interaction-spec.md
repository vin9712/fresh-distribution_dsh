...
> **2026-09-19 更新（D-064~D-069）**：列表新增**客户视角（默认）**，一行 = 客户 + 配送日期；行内二层展开明细单（订单编号/配送点/班次/状态/金额 + 逐单操作），菜品级明细仍进订单明细页。工具栏 `客户视角 / 明细视角` 切换（记忆 `localStorage: sale:listView`），按订单编号搜索自动切明细视角。聚合走后端 `GET /order/sale/customer-page`（分页单位 = 客户行）；子行复用 `GET /order/sale/list`。视觉上：主行按信息权重分级（客户名重、日期/点数淡），进度用彩色圆点 + 计数（非 tag 墙），子区加底色 + 左侧引导线 + 缩进，**班次/备注列仅在有数据时出现**。详见《[销售订单客户视角聚合设计](../01-design/销售订单客户视角聚合设计.md)》。**下文为历史描述，部分已过期**（vxe 批量下拉/生成采购单/生成送货单均已下线，现为批量操作条 + 独立明细入口）。
