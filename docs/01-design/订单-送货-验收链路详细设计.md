...
> **状态**：🟢 已定稿 v1.2（依据基线《[销售订单与送货单关系设计](../销售订单与送货单关系设计.md)》v1.0 的 D-001 ~ D-037 决策，落到库表 / 服务 / 接口 / 页面；v1.1：验收明细粒度改为点级「录入即归属」，废除 diff_alloc 表；v1.2：C1 定稿——实收归验收单、入口仅已配送触发、验收页带来源对照列。2026-08-28 随 T1~T7 实施完成本文转**已定稿**，与实现差异之处以代码为准并记录在[进度文](订单-送货-验收链路开发进度.md)）
>
> 🔗 **送货单客户维度专项设计（D-038 ~ D-042，2026-08-29）**见[送货单客户维度设计](送货单客户维度设计.md)：送货单以「客户 + 配送日期」为第一维度，生成改为三步式预览，并在**客户管理页**体现组单策略与客户维度送货单视图。
> **日期**：2026-08-28
...
`updateDraft` 增强：
1. 双向原因（D-013/G8）：**2026-08-28 经 W0-2.6 修订——仅实收=0（全部拒收）或超收（difference>0）必填 `reasonCode`；部分短收（0<实收<送货）建议但不强制；无差异清空**。正差异存 `acceptance_overage_reason`、负差异存 `acceptance_shortfall_reason`（明细行 `reason_type` 1短收2超收）。
...
| GET | `/order/delivery/group-preview?deliveryDate=&customerId=` | ✅ 已实现（D-038~D-042，详见[送货单客户维度设计](送货单客户维度设计.md)）：生成前**客户维度待生成清单**预览——逐客户返回组单策略与来源（批次快照/客户配置）、三态分支（CREATE/REBUILD/SUPPLEMENT）、待并入订单、预计成单张数/明细行/金额、既有有效单影响面，另返当日未确认草稿提示；**只读不落库**，与生成同源判定（共用 `groupRowsByPoint`/`mergeRows`）；`customerId` 可空=当日全部客户 |
...
> **2026-09-14 修订（状态收敛）**：下表为 S14 时期口径（当时未分配送货单的已确认单可直接编辑）。
> 现行口径已收敛为">**只有「草稿」可直接改单**
> ——已确认单必须先在订单列表「撤回」回草稿，才能在草稿态编辑；撤回本身被验收单/已打印·已送达送货单/已入库采购单阻断。
> 列操作矩阵: 草稿=修改+删除；已确认=撤回+去验收+配送后变更；已验收/已结算=验收单+调整摘要（无修改/删除）。

| 订单状态 | 未进送货单 | 已进有效送货单(PENDING/PRINTED) | 送达后(DELIVERED) |
|---|---|---|---|
| 编辑明细/表头 | ❌ → 先「撤回」回草稿（2026-09-14 收敛；原为 ✅） | ❌（提示先作废送货单） | ❌ → 用新增销售订单补货（D-030） |
| 撤回草稿 | ✅ | ❌ | ❌ |
| 删除草稿 | ✅ | ❌ | ❌ |
| 生成送货单入口 | 勾选多条 ✅ / 手工按客户 ✅ | — | — |
| 验收 | — | — | ✅ 经验收页 |
...
| `order/delivery/index.vue` | 状态 tag 增已作废样式；操作列：作废（弹原因下拉）、送达（PENDING 弹原因弹窗）、来源明细 dialog、补充单 tag、predecessor 链接跳转；**[2026-08-29 D-039]** 工具栏「生成送货单」改为三步式（选日期 → `generatePreviewDrawer.vue` 客户维度待生成清单 → 确认生成） |
...
| `partner/customer/index.vue` | 表单补两字段：组单策略 select + 合并开关；**[2026-08-29 D-040 扩展]** 列表加「送货单组单策略 / 相同商品合并」列 + 按策略筛选，行操作加「送货单」→ 新 `customerDeliveryDrawer.vue`（客户维度：策略来源 + 待生成清单 + 当日单 + 一键生成/补单） |
