...
- S6-1 报表（主控）：ReportMapper（selectDailySaleRows/selectStatementRows，join acceptance+item+delivery+customer+dept，只算 status=1）；ReportService 组装分组（日报=客户+配送点分组实收/损耗数量/损耗金额+商品明细；对账单=按验收单分组+期间合计）；POI XSSFWorkbook 导出两级 Excel（不走 ExcelUtil，多级结构它不支持）；菜单 2087-2089（report:list/query/export）；前端 report/index.vue 双 Tab（展开行明细、空态提示、缺客户警示）；5 个 Mockito 单测。JimuReport 报表模板随 S6-2 一并落地（DESIGN 首版顺序）。
- S6-2 打印落地（主控）：PrintTemplate 补 5 字段（render_engine/bind_type/delivery_point_id/copies/is_default，content=JimuReport 模板ID）；selectBindTemplate 三级绑定 SQL（1客户+配送点 > 2客户 > 3全局默认，bind_type asc+limit 1，停用不参与）；resolveForDeliveryOrder + printInfo 接口；前端打印模板页重写（绑定类型/联数/设计器入口 /jmreport/list?token=）；送货单打印按钮流程（printInfo→确认联数→markPrinted→打开 /jmreport/view/{id}?token=&deliveryOrderId=）。**关键坑**：①vite dev 不代理 /jmreport（补 proxy '/jmreport'→8090）；②JimuReport API 数据集响应必须是 JSON 对象且行数据走 ApiDataConvertAdapter 转换器（bean 名配在 jimu_report_db.api_convert；/print/deliveryHead→{"head":{}}、/print/deliveryData→{"rows":[]}，SecurityConfig 放行）；③模板 querySetting.izDefaultQuery=true 才能 URL 传参自动查询（getQueryInfo→checkParam→show 自动执行，实测两个数据集都带 deliveryOrderId 调用）；④t_print_template 有 data JSON NOT NULL 列，insert 必须带 '{}'；⑤jimu_report id 用固定 2099... 字符串（主键 varchar(32)，INSERT IGNORE 幂等）；⑥模板 JSON 里单值数据集用 ${ds.field}、列表用 #{ds.field}。预置：送货单模板 2099000000000000001 + hd/dd 数据集 + 参数 + 全局默认绑定（s6_2_print_seed.sql）。打印计数@Log 已满足验收 11。
- S6-3 部署（主控）：deploy/ 目录（DEPLOY.md 部署手册、nginx.conf.example HTTPS+静态+反代、backup.sh 每日7天+月度1年、restore.sh 恢复演练、systemd 单元）+ application-prod.yml.example（锁定时长30分钟、禁 devtools/swagger）+ .gitignore 排除 application-prod.yml。**注意**：S6-2 数据集 api_url 写死 8090 开发端口，生产需 UPDATE 为 8080（DEPLOY.md §9 给了 SQL）。验证：bash -n 两个脚本 exit 0；mvn package 出 lin-entry.jar（169MB）；恢复演练只能在 Linux 上执行（文档化步骤，未实际演练）。
- **全链路 E2E（DESIGN 验收 14，主控实测通过）**：`.dsh-e2e/e2e-full-flow.js` 走 API 完整业务链——取号(refresh=true)→建单→确认→采购生成 PC→送货生成 HS→打印（模板解析+计数）→送达→验收建单/录入/提交 YS→订单 ACCEPTED→销售日报命中。真实单据：XD202608170001 / PC20260817001 / HS20260817001 / YS20260817001（测试数据保留在开发库）。**教训**：/order/sale/code 必须 refresh=true（peek 号建单会与序列号冲突且不推进序列，前次残留已清理）；create 接口返回 SaleOrder 对象（data.id）。
- **验收标准 2 补全（打印品名客户映射优先）**：PrintController.deliveryData 按 (customerId,skuId) 查 customer_sku_mapping，有映射用客户叫法覆盖 productName（无映射用快照）；实测 verify-print-alias.js 建映射→打印数据品名变客户叫法→删映射恢复，通过。
- 菜单 ID 已用至 2089，新菜单从 2090 起。
