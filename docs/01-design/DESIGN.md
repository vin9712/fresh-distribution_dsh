...
9. 一张送货单对应一张验收单；验收明细独立保存；实收金额=`actual_quantity × unit_price`；损耗=`actual_quantity − delivered_quantity`（**实收可超送，损耗可为负，须填原因**；2026-08-28 经 W0-2.6 修订：仅实收=0（全部拒收）或超收必填原因，部分短收（0<实收<送货）建议但不强制）；验收总额为结算依据。
...
8. 后端按实收数量×单价重算每行与整单金额；损耗=实收−送货（可为负，负值必填原因）；**2026-08-28 W0-2.6 修订：负差异改为仅实收=0（全部拒收）必填原因，部分短收（0<实收<送货）建议但不强制，超收必填；替代原 D-013/G8「双向差异均必填」**；
