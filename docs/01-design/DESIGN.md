...
- 报价：客户报价（沿用现有流程）、报价模板、配送点报价，均带有效期；
...
7. 取价优先级固定：配送点报价 > 客户报价 > 客户关联报价模板；未命中返回空价，由文员人工填写，不得自动转为免费/零价。
8. 订单行保存下单时的品名、规格、单位、单价快照；送货单打印品名**客户映射叫法优先**（无映射用我方品名）；主数据与报价变化不回写历史订单。
9. 一张送货单对应一张验收单；验收明细独立保存；实收金额=`actual_quantity × unit_price`；损耗=`actual_quantity − delivered_quantity`（**实收可超送，损耗可为负，须填原因**）；验收总额为结算依据。
...
| 报价管理 | 客户报价/模板/配送点报价、有效期、取价 | **t_product_sku_quote(+detail)**（改造为带有效期客户报价）、price_template、price_template_sku、delivery_point_price |
| 订单管理 | 录单、快照、追溯、加退换 | **t_sale_order(+detail)**（改造状态机与字段）、order_adjustment |
| 采购管理 | 汇总/手工采购、成本 | purchase_order、purchase_item |
| 配送管理 | 送货单、打印、送达 | **t_delivery_order(+detail)**（改造状态机）、**t_print_template/t_print_task**（按打印方案改造） |
...
- 送货单标记送达 → 订单 DELIVERED（此后调整走 order_adjustment）；
- 提交验收单 → ACCEPTED；
...
输入：`customerId`、`deliveryPointId`、`skuId`、`deliveryDate`。

1. 查询配送点报价（PUBLISHED 且 `effective_date ≤ deliveryDate ≤ expire_date`），区间内取最新；
2. 未命中查客户报价（同规则）；
3. 未命中查客户关联的报价模板 SKU 价（同规则）；
4. 仍未命中返回空价，前端明确提示文员人工填写。
...
8. 后端按实收数量×单价重算每行与整单金额；损耗=实收−送货（可为负，负值必填原因）；
