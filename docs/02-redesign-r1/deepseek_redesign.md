...
>
> ⚠️ **`[2026-08 W0-1]` 本文 §3.8/§3.10/§6.4 涉及的「配送点覆盖价、报价模板」价格层级已废弃**：
> 订单仅使用客户正式报价，未命中由文员手工定价；相关表已删除（`sql/w01_price_simplify.sql`）。
> 现行权威口径以《客户端化ERP优化蓝图》§2「价格层级」与《DESIGN.md》§8 为准，下文相关章节仅作历史追溯。
